...
public class Lexer {
    ...
    public static void main(String args[]) {
        ...
        String text = null;
        try {
            //lex = new Lexer("factorial.x");
            //lex = new Lexer("simple.x");
            lex = new Lexer(args[0]);
            while (true) {
                tok = lex.nextToken();
                System.out.format("%-13s%s%-10d%s%-10d%s%-10d\n",tok.toString(),"left: ", tok.getLeftPosition(),"right: ",tok.getRightPosition(),"Line: ", tok.getLineNumber()); //lex.source.getLineno());
                //text = lex.source.getLineno() + ;
            }
            
            
          //  while (true) {
          //      System.out.format("1:", lex.source.getLineno());
          //  }
        } catch (Exception e) {
        }
    ...
    public Token newNumberToken(String number, int startPosition, int endPosition) {
        return new Token(
                startPosition, endPosition, source.getLineno(),
